        ...
        for i in range(len(path) - 1):
            ...
            for j in range(1, additional_smoothing_points + 1):
                new_path.append(
                    (
                        path[i][0]
                        + j * (path[i + 1][0] - path[i][0]) / (additional_smoothing_points + 1),
                        path[i][1]
                        + j * (path[i + 1][1] - path[i][1]) / (additional_smoothing_points + 1),
                    )
                )
        ...
        while change >= tolerance:
            ...
            for i in range(1, len(new_path) - 1):
                for j in range(2):
                    ...
                    smoothed_path[i] = (
                        smoothed_path[i][0]
                        + data_weight * (new_path[i][0] - smoothed_path[i][0])
                        + smooth_weight
                        * (
                            smoothed_path[i - 1][0]
                            + smoothed_path[i + 1][0]
                            - 2.0 * smoothed_path[i][0]
                        ),
                        smoothed_path[i][1]
                        + data_weight * (new_path[i][1] - smoothed_path[i][1])
                        + smooth_weight
                        * (
                            smoothed_path[i - 1][1]
                            + smoothed_path[i + 1][1]
                            - 2.0 * smoothed_path[i][1]
                        ),
                    )
        ...
        for i, node in enumerate(nodes):
            for other_node in nodes[i + 1 :]:
                ...
                if distance <= connection_distance:
                    if not self._map.crosses([node, other_node]):
